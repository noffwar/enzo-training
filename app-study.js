export const createStudyView = (deps) => {
  const {
    html, useState, useEffect, useCallback, useRef, supabase,
  ...
  return function StudyView({ session, timer, onToggleTimer, onResetTimer }) {
    ...
    const loadStudy = useCallback(async () => {
      ...
      try {
        ...
        if (missing.length && session?.user?.id) {
        ...
        console.error('[StudyView] Load error:', e);
    ...
    const sendTopicToTasks = async (row, topic) => {
      setError('');
      setNotice('');
      if (!session?.user?.id) { setError('Inicia sesión para enviar tareas.'); return; }
      try {
        const payload = {
          user_id: session.user.id,
                      ...
                      <div style="padding:0 14px 14px;display:flex;flex-direction:column;gap:8px;border-top:1px solid #1E2D45;">
                        <div style="padding-top:10px;display:flex;flex-direction:column;gap:6px;">
                          ${topics.length === 0 ? html`
                            <p style="margin:0;color:#64748b;font-size:12px;">Todavía no hay temas cargados.</p>
