export const createTodayDashboard = ({
  ...
  return function TodayDashboard({
    ...
    const [summary, setSummary] = useState({
      ...
      pantryLow:0,
      medNextLabel:'Sin datos de salud',
      medNextDetail:'',
      backupDue:false,
      backupLabel:''
    });
    ...
    useEffect(() => {
      loadWeather();
      const onFocus = () => loadWeather();
      const onVisible = () => { if(document.visibilityState === 'visible') loadWeather(); };
      window.addEventListener('focus', onFocus);
      document.addEventListener('visibilitychange', onVisible);
      return () => {
        window.removeEventListener('focus', onFocus);
    ...
    const medDashboardStatus = useMemo(() => {
      const medsView = getMedicationStatusForView({
        selectedDateKey,
        medsState: tracker?.meds || {},
        now: new Date()
      });
      const roaccTime = summary.meds?.last_roaccutan_at
        ? new Date(summary.meds.last_roaccutan_at).toLocaleTimeString('es-AR',{hour:'2-digit',minute:'2-digit'})
        : '-';
      const dinnerTime = summary.meds?.last_dinner_meds_at
        ? new Date(summary.meds.last_dinner_meds_at).toLocaleTimeString('es-AR',{hour:'2-digit',minute:'2-digit'})
        : '-';

      if(!medsView.roaccutanDone) {
        return {
          label:'Roaccutan pendiente',
          detail: medsView.viewDateKey === medsView.calendarTodayKey ? 'Mediodia' : `Sin marcar en ${medsView.viewDateKey}`
        };
      }
      ...
      return {
        label: medsView.dinnerDone ? 'Cena registrada para ese dia' : 'Cena no corresponde ahora',
        detail: medsView.dinnerDone ? `Ultimo registro cena: ${dinnerTime}` : `Vista de ${medsView.viewDateKey}`
      };
    }, [tracker, selectedDateKey, summary.meds]);
    ...
    return html`
      <div class="glass-card" style="padding:12px 14px;display:flex;flex-direction:column;gap:10px;">
        ...
        <div style="display:grid;grid-template-columns:repeat(2,1fr);gap:8px;">
          <${DashboardActionCard} onClick=${onOpenStudy} title="Estudio" value=${loading ? '...' : summary.studyPending} detail=${`Pendientes de ${summary.studySubjects} materias - ${summary.studyDone} hechos`} border="rgba(99,102,241,0.25)" background="rgba(99,102,241,0.08)" accent="#A5B4FC" />
        ...
        <div style="padding:10px;border-radius:10px;background:rgba(10,15,30,0.45);border:1px solid #1E2D45;">
          ...
          ${!loading && summary.nextTask && html`
            <div style="display:flex;gap:6px;flex-wrap:wrap;margin-top:10px;">
              <button onClick=${()=>quickUpdateTask(summary.nextTask,'done')} style="padding:6px 10px;border-radius:8px;border:1px solid rgba(16,185,129,0.35);background:rgba(16,185,129,0.12);color:#86EFAC;font-size:11px;font-weight:800;font-family:'Barlow Condensed',sans-serif;cursor:pointer;letter-spacing:0.05em;">
                HECHA
              </button>
              <button onClick=${()=>quickUpdateTask(summary.nextTask,'archived')} style="padding:6px 10px;border-radius:8px;border:1px solid rgba(148,163,184,0.35);background:rgba(148,163,184,0.12);color:#CBD5E1;font-size:11px;font-weight:800;font-family:'Barlow Condensed',sans-serif;cursor:pointer;letter-spacing:0.05em;">
                ARCHIVAR
              </button>
              <button onClick=${onOpenTasks} style="padding:6px 10px;border-radius:8px;border:1px solid rgba(99,102,241,0.35);background:rgba(99,102,241,0.12);color:#A5B4FC;font-size:11px;font-weight:800;font-family:'Barlow Condensed',sans-serif;cursor:pointer;letter-spacing:0.05em;">
                ABRIR
              </button>
            </div>
          `}
